...
const AdminReservationsPage = () => {
  ...
  const openEdit = (ev: CalendarEvent) => {
    ...
    const kind =
      res.kind ?? (res.userId ? "user" : res.guestName ? "guest" : "break");
  ...
  const saveReservation = async () => {
    ...
    try {
      ...
      if (editingId) {
        ...
        if (
          !isWithinWorkingHours(dayConfig, format(start, "HH:mm"), duration)
        ) {
      ...
      if (editingId) {
        ...
        const oldSlotId =
          existing?.date && existing?.startTime
            ? buildSlotId(existing.date, existing.startTime)
            : null;
  ...
  const openEditFromInfo = () => {
    ...
    const event = events.find((e) => e.id === infoReservation.id);
    if (!event) return;
    setInfoReservationId(null);
    openEdit(event);
  };
                ...
                <select
                  ...
                  onChange={(e) =>
                    setForm({
                      ...
                      userId: e.target.value === "user" ? form.userId : "",
                      guestName:
                        e.target.value === "guest" ? form.guestName : "",
                      serviceId:
                        e.target.value === "break" ? "" : form.serviceId,
                      cardColor:
                        DEFAULT_COLOR_BY_KIND[
                          e.target.value as FormState["kind"]
                        ],
                    })
                  }
                  className="w-full rounded border border-gray-300 px-3 py-2 text-sm"
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-lg rounded-2xl border border-blue-100 bg-white p-6 shadow-2xl">
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              <button
                type="button"
                onClick={openEditFromInfo}
                className="rounded-lg bg-[#1F50FF] px-4 py-2.5 text-base font-semibold text-white"
              >
                Izmeni
              </button>
            </div>
