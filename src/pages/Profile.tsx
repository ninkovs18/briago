  ...
  const handleProfileSave = async () => {
    ...
    try {
      ...
      if (profileEdit.phone.trim() && !isValidSerbianPhone(normalizedPhone)) {
        setProfileError(
          "Unesi važeći broj telefona (npr. +381641234567 ili 0641234567).",
        );
        return;
      }
                ...
                <div className="space-y-4">
                  {!userProfile?.isAdmin && !userProfile?.verified && (
                    <div className="rounded-lg border border-yellow-500/40 bg-yellow-500/10 px-4 py-3 text-sm text-yellow-200">
                      Profil nije verifikovan. Kada admin potvrdi nalog, moći
                      ćeš da zakažeš termin.
                    </div>
                  )}
                  <div>
              ...
              {loading ? (
                ...
                <div className="space-y-4">
                  {upcomingAppointments.map((appointment) => (
                    <div
                      key={appointment.id}
                      className="relative bg-barbershop-dark rounded-xl border border-gray-700/60 p-5 pb-12"
                    >
                      <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                        <div className="flex items-center gap-4">
                          <div className="rounded-lg bg-barbershop-gold/15 px-3 py-2 text-center">
                            <div className="text-sm text-barbershop-gold">
                              {weekdayLabel(appointment.date)}
                            </div>
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                {pastAppointments.slice(0, 5).map((appointment) => (
                  <div
                    key={appointment.id}
                    className="bg-barbershop-dark rounded-xl border border-gray-700/60 p-5"
                  >
                    <div className="flex items-center gap-4">
